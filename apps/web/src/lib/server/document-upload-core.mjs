...
function detectFormat(bytes) {
  return Object.entries(formats).find(([, format]) => format.signature(bytes))?.[0] ?? null;
}
...
export function validateUpload({ name, mimeType, bytes }) {
  ...
  const extension = detectFormat(bytes);
  const format = extension ? formats[extension] : null;
  if (!format) return { ok: false, status: 415, error: "Formato no reconocido. Solo se admiten PNG, JPG, WEBP o PDF." };
  const declaredMime = String(mimeType ?? "").toLowerCase();
  if (declaredMime && declaredMime !== "application/octet-stream" && declaredMime !== format.mime) return { ok: false, status: 415, error: "El tipo de archivo no coincide con su contenido." };
