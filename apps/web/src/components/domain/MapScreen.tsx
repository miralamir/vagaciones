...
export function MapScreen() {
  ...
  return <AppShell><section className="grid gap-3">
    ...
    <SectionCard title={filter === "today" ? `Lugares del Dia ${context.activeDay.day}` : "Lugares utiles"}>{visiblePlaces.length ? <div className="grid gap-3">{visiblePlaces.map((place) => <PlaceCard key={place.id} onDelete={"manual" in place ? () => removePlace(place.id) : undefined} place={place} />)}</div> : <p className="rounded-md bg-mist px-3 py-3 font-bold text-ink/65">No hay lugares para este filtro todavia.</p>}</SectionCard>
  </section></AppShell>;
}
