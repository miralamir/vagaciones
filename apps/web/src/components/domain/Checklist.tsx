...
export function Checklist({ day, items }: { day: number; items: ChecklistItem[] }) {
  ...
  const updateDone = (next: string[]) => {
    setDone(next);
    try {
      window.localStorage.setItem(doneKey, JSON.stringify(next));
    } catch {
      setMessage("No se pudo guardar el estado de la checklist en este navegador.");
    }
  };
  ...
  const addPersonalItem = (event: FormEvent<HTMLFormElement>) => {
    ...
    const next = [...getPersonalChecklistItems(targetDay), { id: `personal-${Date.now()}-${Math.random().toString(36).slice(2)}`, label: text, priority, category: category.trim() || undefined, createdAt: new Date().toISOString() }];
    if (!savePersonalChecklistItems(targetDay, next)) {
      setMessage("No se pudo guardar el item en este navegador.");
      return;
    }
  ...
  const removePersonalItem = (id: string) => {
    ...
    if (!savePersonalChecklistItems(day, next)) setMessage("No se pudo guardar el cambio en este navegador.");
